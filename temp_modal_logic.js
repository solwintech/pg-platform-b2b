const [showOtpModal, setShowOtpModal] = useState(false);
  ...
  const fetchProperties = async () => {
    ...
    try {
      ...
      if (appliedFilters.city) {
        fetchedProperties = fetchedProperties.filter(p => p.city?.toLowerCase().includes(appliedFilters.city.toLowerCase()));
      }
  ...
  const openOtpModal = async (property, action) => {
    ...
    if (authService.isAuthenticated() && action === 'call') {
                ...
                properties.map(property => (
                  <Card key={property._id} className="pc-card">
                    <div className="pc-body">
                      ...
                      {/* ── Details ── */}
                      <div className="pc-details" onClick={() => handleViewDetails(property._id)} style={{ cursor: 'pointer' }}>
